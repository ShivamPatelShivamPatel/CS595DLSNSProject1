...
if [ -z "${TOPOLOGY}" ]
then
  TOPOLOGY=${HANGARGAMES}/networks/FederatedElectionCount/FederatedElectionCount.yml
fi
...
function selftest {
  sudo -E python3 ${HANGARGAMES}/BMv2/start_flightplan_mininet.py ${TOPOLOGY} \
          ...
          --showExitStatus \
     --fg-host-prog "${START_CFG}" \
     --fg-host-prog "b1: ping -c 1 192.0.0.5" \
     --fg-host-prog "b1: ping -c 1 192.0.0.6" \
     --fg-host-prog "b1: ping -c 1 192.0.0.7" \
     --fg-host-prog "b1: ping -c 1 192.0.0.8" \
     --fg-host-prog "b1: ping -c 1 192.0.0.9" \
     --fg-host-prog "b1: ping -c 1 192.0.0.11" \
          2> $LOG_DUMPS/flightplan_mininet_log.err
}
